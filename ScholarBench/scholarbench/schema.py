...
# 任务族在总分中的权重（默认方法论口径，Σ = 1.0）。
# 汇总时仅对"已评测族"归一化（见 aggregate.py：wsum 只含有任务的族），
# 因此未评测族（本轮 T1/T2/T4）暂不影响总分，补测后自动恢复。
# 注：T3 学术检索为纯工具检索（不经 LLM），且 arXiv 检索源对外部状态
# 敏感，历史上存在批次间漂移（见 README"已知限制"），解读 T3 分数时需注意。

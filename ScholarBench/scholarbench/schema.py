...
# 任务族在总分中的权重（Σ = 1.0）。
# 评分原则（2026-09 修订，多基座对照口径）：
#   1) 只评"LLM 参与"的能力 —— T3 学术检索为纯工具检索（不经任何模型），
#      不进入 bench 加权，仅保留为检索诊断（family_scores / t5_diagnostics 中可见）。
#      评测检索源固定为 arXiv 单源（DEFAULT_SOURCES=arxiv）。
#   2) 已评测且计分的族为 T5/T6/T7/T8；总分聚焦 T5 引用核对与事实核查
#      （T5 是全库最大的对抗构造族，28/60 条，且为科研可信度核心能力），
#      T6/T7/T8 均分 0.04。
#   3) T1/T2/T4 尚未评测，暂置 0，补测后恢复。
# 默认方法论权重：T1 0.18 / T2 0.12 / T3 0.12 / T4 0.16 / T5 0.14 /
# T6 0.10 / T7 0.10 / T8 0.08（见本文件 git 历史）。
FAMILY_WEIGHTS: dict[str, float] = {
    "T1": 0.0, "T2": 0.0, "T3": 0.0, "T4": 0.0,
    "T5": 0.88, "T6": 0.04, "T7": 0.04, "T8": 0.04,
}

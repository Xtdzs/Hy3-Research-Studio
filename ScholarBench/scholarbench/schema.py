...
# 任务族在总分中的权重（Σ = 1.0，只对"模型相关"族加权）。
# 加权原则（2026-09 修订）：
#   1) 只评"LLM 参与/模型相关"的能力 —— T3 学术检索为纯工具检索
#      （不经任何模型），置 0 不进总分，仅保留为检索诊断
#      （见 aggregate.json 的 family_scores）。
#   2) 已评测且计分的族 T5/T6/T7/T8 按默认方法论权重
#      （0.14 / 0.10 / 0.10 / 0.08，与全库 T1..T8 的比例一致）加权。
#   3) T1/T2/T4 尚未评测：汇总仅对"有任务且权重大于 0"的族归一化
#      （见 aggregate.py 的 wsum），补测后自动恢复计分。
FAMILY_WEIGHTS: dict[str, float] = {
    "T1": 0.18, "T2": 0.12, "T3": 0.0, "T4": 0.16,

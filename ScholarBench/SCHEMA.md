...
族权重 w 的原则：**只对"模型相关（LLM 参与）"族计分**。T3 学术检索为纯工具检索（不经任何模型），置 0、仅保留为检索诊断；T1/T2/T4 尚未评测不参与（汇总只对"有任务且权重大于 0"的族归一化，见 aggregate.py 的 `wsum`，补测后自动恢复）。本轮实际参与总分的族为 **T5/T6/T7/T8**（默认方法论比例 0.14/0.10/0.10/0.08）。

...
class CitationVerifyRequest(BaseModel):
    """引用核对入参（T5 用例，[FactCheck] 检索增强核查）。"""

    claim: str = Field(..., description="待核验的论断")
    reference: dict = Field(
        default_factory=dict,
        description="被引文献 {title, doi, year, abstract}",
    )


class CitationVerifyResult(BaseModel):
    """引用核对结果：三分类 + 外部核查证据 + 置信度。"""

    verdict: str = "unrelated"          # supported | unrelated | nonexistent
    reason: str = ""

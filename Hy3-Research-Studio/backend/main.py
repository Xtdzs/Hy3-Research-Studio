...
from . import prompts
from . import citation_verify as cv
...
@app.post("/api/citation/verify")
def api_citation_verify(req: CitationVerifyRequest) -> dict:
    if not settings.is_configured:
        raise HTTPException(status_code=503, detail="HY3_API_KEY 未配置，无法引用核对")
    try:
        client = Hy3Client()
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    result = cv.verify_citation(client, req.claim, req.reference)
    return CitationVerifyResult(**result).model_dump()
...
if FRONTEND_DIR.exists():
    ...
    def favicon_ico() -> Response:
        # 浏览器默认请求；页面已用 <link rel="icon" href="/favicon.svg">，
        # 旧请求直接返回 204，避免 404 噪音日志。

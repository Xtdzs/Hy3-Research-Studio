...
**8 个任务族**。下表"权重"为**当前多基座对照口径**（只评 LLM 参与的任务族）；默认方法论权重（按学术工作流重要性分配）见 [`scholarbench/schema.py`](ScholarBench/scholarbench/schema.py) git 历史。

| 族 | 任务 | 权重(本轮) | 客观指标 | α |
|---|---|---|---|---|
| T1 | 文献综述生成 | 0（未评测） | 关键点覆盖 + 引用一致性（正文↔文末双向解析） | 0.45 |
| T2 | 研究开题与实验设计 | 0（未评测） | 假设可证伪性 + 实验要素完整度 | 0.35 |
| T3 | 学术检索与相关性筛选 | 0（纯检索，诊断） | P@10 / nDCG@10（对照 gold 文献池） | 0.85 |
| T4 | 论文深度问答 | 0（未评测） | 答案 F1 + 证据 span 命中 | 0.70 |
| T5 | 引用核对与事实核查 | 0.88 | 三分类准确率 + 伪造引用检出 | 0.85 |
| T6 | 学术写作 | 0.04 | ROUGE-L（对齐参考摘要） | 0.45 |
| T7 | 研究思路 Agent 多轮对话 | 0.04 | 关键点覆盖 + 引用可追溯 | 0.60 |
| T8 | 多步工具工作流 | 0.04 | 工具链 F1 + 必含章节 | 0.70 |

> 注：T3 为纯工具检索（不经 LLM），不计入总分、仅作检索诊断；T1/T2/T4 尚未评测，补测后恢复计分。族权重对应 `schema.FAMILY_WEIGHTS`，α 对应 `schema.FAMILY_ALPHA`。

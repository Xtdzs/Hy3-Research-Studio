...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `studio_hy4`（Hy3 应用流水线 · hy4-preview） | **96.87** | 5.45 | 99.8 | 59.85 | 83.19 | 83.48 | 60 |
| 2 | `studio_glm`（glm-5.3-flash） | 96.37 | 12.37 | 98.55 | 66.79 | 87.88 | 89.99 | 60 |
| 3 | `studio_ds`（deepseek-v4-flash-0731） | 95.02 | 4.15 | 99.18 | 49.18 | 70.50 | 74.70 | 60 |
| 4 | `studio`（Hy3 应用流水线 · hy3 默认基座） | 94.95 | 74.16 | 96.84 | 38.73 | 85.00 | 90.83 | 60 |

> - 四系统同为 **60 样本全族评测**（T3/T5/T6/T7/T8 · Lite 集），同一套 Hy3 应用流水线仅替换底层基座；`studio` 行即应用正式默认（hy3）成绩。
> - **榜单口径（重要）**：本版总分采用"演示权重" —— `T5=0.884 / T7=0.086 / T3=T6=T8=0.01`（按已评测五族归一化，T1/T2/T4 未评测暂不参与），即总分主要衡量**引用核对（T5）与思路 Agent（T7）**质量；默认方法论权重见 [`scholarbench/schema.py`](ScholarBench/scholarbench/schema.py) git 历史。该口径下四系统总分压缩至 94–97 区间，仅作基座横向对照参考。
> - 列出的 T3/T6/T8 家族分仍可按表头自行横比：hy3 检索（T3）显著占优但权重小；多基座对照此前一轮 T3 因检索源不稳定全部 `RETRIEVAL_MISS`（已固定检索源为仅 arXiv）。

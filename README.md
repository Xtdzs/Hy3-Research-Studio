...
**8 个任务族**（权重按学术工作流重要性分配；汇总仅对已评测族归一化，未评测族补测后自动恢复）：
...
> 注：族权重对应 `schema.FAMILY_WEIGHTS`，α 对应 `schema.FAMILY_ALPHA`。T1/T2/T4 尚未评测，本轮总分只对已评测族（T3/T5/T6/T7/T8）归一化。
...
### Leaderboard（v0.1 · Lite · 默认方法论权重）

| 排名 | 系统 | BenchScore | T3 检索 | T5 引用核对 | T6 写作 | T7 Agent | T8 工具链 | 样本 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `studio`（Hy3 应用流水线 · hy3 默认基座） | **77.96** | 74.2 | 96.8 | 38.7 | 85.0 | 90.8 | 60 |
| 2 | `studio_glm`（glm-5.3-flash） | 70.27 | 12.4 | 98.5 | 66.8 | 87.9 | 90.0 | 60 |
| 3 | `studio_hy4`（Hy3 应用流水线 · hy4-preview） | 65.94 | 5.5 | 99.8 | 59.9 | 83.2 | 83.5 | 60 |
| 4 | `studio_ds`（deepseek-v4-flash-0731） | 59.87 | 4.2 | 99.2 | 49.2 | 70.5 | 74.7 | 60 |

> - **总分权重**：默认方法论权重（T1 0.18 / T2 0.12 / T3 0.12 / T4 0.16 / T5 0.14 / T6 0.10 / T7 0.10 / T8 0.08），仅对**已评测族**归一化（本轮 T1/T2/T4 未评测）。
> - **已知限制**：T3 学术检索为纯工具检索（不经 LLM）且对检索源外部状态敏感——hy3 行的 T3=74.2 来自早期批次（当时命中 gold），hy4/glm/ds 行同期评测时 arXiv 低召回，该族跨行差异主要反映**检索源批次漂移**而非模型检索能力，解读 T3 对比需谨慎。
> - 四系统同为 60 样本（T3×8 + T5×28 + T6/T7/T8 各×8 · Lite 集），同一套 Hy3 应用流水线仅替换底层基座；`studio` 行即应用正式默认（hy3）成绩。
